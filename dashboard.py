...
st.caption(
    "Create a controlled release request for credit approval. "
    "No pledged shares are changed by this action."
)
...
try:
    ...
    if release_borrowers:
        ...
        # ----------------------------------------------------
        # Available securities for selected borrower
        # ----------------------------------------------------
        ...
        if "release_result" in st.session_state:
            ...
            if (
                ...
                result_columns[0].metric(
                    "Current Cover",
                    f"{release_result['Current Cover']:.2f}x"
                )

                result_columns[1].metric(
                    "Cover After Release",
                    f"{release_result['Cover After Release']:.2f}x"
                )

                result_columns[2].metric(
                    "Maximum Safe Shares",
                    f"{release_result['Maximum Safe Release Shares']:,}"
                )

                result_columns[3].metric(
                    "Release Value",
                    format_crore(
                        release_result["Release Value"]
                    )
                )

                st.write(
                    f"Requested shares: "
                    f"**{release_result['Requested Shares']:,}**"
                )

                st.write(
                    f"Collateral after release: "
                    f"**{format_crore(release_result['Collateral After Release'])}**"
                )
                ...
                if release_result["Status"] == "APPROVED":

                    st.success(
                        "🟢 RELEASE REQUEST PERMISSIBLE"
                    )

                else:

                    st.error(
                        "🔴 RELEASE REQUEST NOT PERMISSIBLE"
                    )
                ...
                st.warning(
                    "⚠️ This is only a release recommendation. "
                    "Pledged shares are NOT changed automatically. "
                    "Final release requires credit approval."
                )
    ...
    st.warning(
        "Collateral release request could not be calculated: "
        + str(e)
    )
